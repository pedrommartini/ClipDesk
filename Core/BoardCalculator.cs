...
/// <summary>Small deterministic calculator used by the native board widget.</summary>
public static class BoardCalculator
{
    public static (string Expression, string Display) Press(string? expression, string key)
    {
        ...
        if (key == "back")
        {
            ...
            return (expression, expression.Length == 0 ? "0" : expression);
        }
        if (key == "=")
        {
        ...
        {
            if (!TryEvaluate(expression, out var value)) return (expression, "Erro");
            var result = (value / 100d).ToString("0.############", CultureInfo.InvariantCulture);
            return (result, result);
        }
        if (key == "±")
        {
        ...
        if (token.Length != 1 || !"0123456789.+-*/".Contains(token, StringComparison.Ordinal))
            return (expression, expression.Length == 0 ? "0" : expression);
        if ("+-*/".Contains(token, StringComparison.Ordinal) && (expression.Length == 0 || "+-*/".Contains(expression[^1])))
        {
            if (token == "-" && expression.Length == 0) expression = "-";
            else if (expression.Length > 0) expression = expression[..^1] + token;
        }
        else if (token == "." && CurrentNumber(expression).Contains('.')) { }
        else expression += token;
        return (expression, expression.Length == 0 ? "0" : expression.Replace("*", "×").Replace("/", "÷"));
    }
    ...
    private static string CurrentNumber(string expression)
    {
        var index = expression.LastIndexOfAny(['+', '-', '*', '/']);
        return index < 0 ? expression : expression[(index + 1)..];
    }
    ...
    private sealed class Parser(string source)
    {
        ...
        {
            SkipWhitespace();
            var sign = Take('-') ? -1d : Take('+') ? 1d : 1d;
            SkipWhitespace();
            var start = _index;
            while (_index < source.Length && (char.IsDigit(source[_index]) || source[_index] == '.')) _index++;
            if (start == _index || !double.TryParse(source[start.._index], NumberStyles.Float, CultureInfo.InvariantCulture, out var value))
